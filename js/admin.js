...
function switchTab(tab) {
  ...
  if (tab === 'students') { populateGradeDropdowns(); renderStudentTable(); }
  if (tab === 'scores')   { populateGradeDropdowns(); renderScoreTable(); }
}
...
function addStudent() {
  ...
  for (var i = 0; i < students.length; i++) { if (students[i].id === id) { showToast('Student ID already exists!', 'error'); return; } }
  students.push({ id:id, name:name, grade:grade, room:room, pw:pw });
...
function openEditStudent(id) {
  var s = _findStudent(id); if (!s) return;
...
function saveEditStudent() {
  ...
  var students = getStudents();
  if (newId !== origId) {
    for (var i = 0; i < students.length; i++) { if (students[i].id === newId) { showToast('That ID already exists.', 'error'); return; } }
  }
  for (var j = 0; j < students.length; j++) {
    if (students[j].id === origId) { students[j] = { id:newId, name:name, grade:grade, room:room, pw:pw }; break; }
  }
  saveStudents(students);
...
function clearAllStudents() {
  ...
  saveStudents([]); updateSfRooms(); renderStudentTable();
  showToast('All students deleted.', 'success');
}

function _findStudent(id) {
...
function handleFileImport(input) {
  ...
  if (ext === 'csv') {
    ...
    showToast('Use .csv or .xlsx files only.', 'error');
  }
  input.value = '';
}

// Columns: No., ID, Name, Level, Room, Password  (header row auto-skipped)
function parseImportData(text) {
  var lines = text.split(/\r?\n/).filter(function(l) { return l.trim(); });
  var students = getStudents(); var added = 0, skipped = 0;
  lines.forEach(function(line, idx) {
    ...
    if (!id || !grade) return;
    var exists = false;
    for (var i = 0; i < students.length; i++) { if (students[i].id === id) { exists = true; break; } }
    if (exists) { skipped++; return; }
    students.push({ id:id, name:name, grade:grade, room:room, pw:pw }); added++;
  });
  saveStudents(students); updateSfRooms(); renderStudentTable();
  showToast('Imported: ' + added + ' | Skipped: ' + skipped, 'success');
}

// Paste format: ID, Name, Level, Room, Password  (no No. column)
...
// ── Score filter dropdown: populate game names from registry ──
